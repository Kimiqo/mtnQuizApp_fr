...
import { Users, RotateCcw, BookOpen } from "lucide-react";
...
export default function RoundFour() {
  ...
  const availableTotal = questions.round4.filter((q) => !q.isUsed);
  const available = availableTotal.filter((q) => q.difficulty === difficulty);
  const total = questions.round4.filter((q) => q.difficulty === difficulty).length;
  ...
  const draw = useCallback(() => {
    ...
    clearBuzzState();
    playSound("draw");
    updateBroadcast({ questionText: null, questionId: null, isShuffling: true, round: 2 });
  ...
  }, [currentQ, showAnswer, broadcast.buzzedTeamId, awardBuzzedTeam, updateBroadcast]);
    ...
    <div className="flex flex-1 flex-col gap-4 overflow-hidden p-5">
      ...
      <div className="flex shrink-0 items-center justify-between">
        <div>
          ...
          <p className="text-xs" style={{ color: "#666", fontFamily: "var(--font-mono)" }}>Open floor · buzzer · 2 pts first correct · HOST VIEW</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setCurrentQ(null); setShowAnswer(false); clearBuzzState(); }}
            className="flex h-9 w-9 items-center justify-center rounded-sm border transition-colors hover:border-orange-400/30"
      ...
      <div className="flex flex-1 gap-4 min-h-0">
        ...
        {/* Speed race: award buzzer winner panel */}
        {currentQ && showAnswer && (
          <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} className="flex w-64 shrink-0 flex-col gap-3">
            <p className="text-[10px] uppercase tracking-widest" style={{ color: "#FFCC00", fontFamily: "var(--font-mono)" }}>
              <Users size={10} className="inline mr-1" />Award Winner
            </p>
            {broadcast.buzzedTeamId ? (
              <div className="flex flex-col gap-2">
                <p className="text-sm text-white font-bold mb-1">
                  Buzzed: {teams.find(t => t.id === broadcast.buzzedTeamId)?.name}
                </p>
                <button onClick={handleCorrect}
                  className="rounded-sm px-4 py-3 text-xs font-black flex items-center justify-center gap-1.5"
                  style={{ background: "#22C55E", color: "#000", fontFamily: "var(--font-display)", fontSize: "14px" }}
                >
                  ✅ CORRECT (+2) <kbd className="hidden sm:inline-block rounded border border-black/20 px-1 py-0.5 text-[9px] bg-black/10">C</kbd>
                </button>
                <button onClick={handleIncorrect}
                  className="rounded-sm px-4 py-3 text-xs font-black flex items-center justify-center gap-1.5"
                  style={{ background: "#EF4444", color: "#000", fontFamily: "var(--font-display)", fontSize: "14px" }}
                >
                  ❌ WRONG (Pass) <kbd className="hidden sm:inline-block rounded border border-black/20 px-1 py-0.5 text-[9px] bg-black/10">W</kbd>
                </button>
              </div>
            ) : (
              <p className="text-xs text-neutral-500">No team buzzed in yet. Awaiting buzz...</p>
            )}
          </motion.div>
        )}
      </div>
